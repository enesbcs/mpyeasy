...
class SX127x:
    ...
    def __init__(self,
        ...
        if "dio_0" in self._pins:
            try:
             ...
             self._pin_rx_done = False
        else:
            self._pin_rx_done = False
        if self._pin_rx_done==False:
           print("IRQ not registered!")
        if "ss" in self._pins:
            self._pin_ss = Pin(self._pins["ss"], Pin.OUT)
        else:
            raise Exception('NSS not setted')
        if "led" in self._pins:
            self._led_status = Pin(self._pins["led"], Pin.OUT)
        if "rst" in self._pins:
            self._pin_rst = Pin(self._pins["rst"], Pin.OUT)
    ...
    def set_frequency(self, frequency):
        ...
        fq2 = frequency*0.016384
        freq_table = [ (int(fq2) >> 16), ((int(fq2) >> 8) & 0xFF), (int(fq2)  & 0xFF) ]
        self.write_register(REG_FRF_MSB, freq_table[0])
        self.write_register(REG_FRF_MID, freq_table[1])
        self.write_register(REG_FRF_LSB, freq_table[2])

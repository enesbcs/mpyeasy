  ...
  if tbuffer[0] == 255:
   if tbuffer[1] == 1:
    ...
    if nlen<0:
     nlen=1
    try:
     cdata = struct.unpack_from('<BBBB6BHBBB'+str(nlen)+'s',self.buffer)
    except Exception as e:
     print(e)
...
#    print(array_alpha)
    ...
    if explen+2==len(tbuffer):
     correction = 1
     try:
      cdata = struct.unpack_from('<4B2H2BH'+str(nlen)+'f',self.buffer)
     except Exception as e:
      print(e)
     self.pktlen = 12+(int(nlen)*4)
    else:
     try:
      cdata = struct.unpack_from('<4B2H2B'+str(nlen)+'f',self.buffer)
     except Exception as e:
      print(e)
    ...
    if pglobals.VARS_PER_TASK<nlen:
     nlen = pglobals.VARS_PER_TASK
